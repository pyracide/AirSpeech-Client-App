...
class RtspProcessor(
    ...
    private fun createClientListener(thread: RtspThread) = object: RtspClient.RtspClientListener {

        override fun onRtspConnecting() {
            if (rtspThread !== thread) return
            if (DEBUG) Log.v(TAG, "onRtspConnecting()")
            uiHandler.post {
                if (rtspThread === thread) {
    ...
    inner class RtspThread: Thread() {
        ...
        fun stopAsync() {
            ...
            try {
                ...
                Log.e(TAG, "Failed to close socket in stopAsync", e)
            }
            // Wake up sleep() code
            interrupt()
        }
        ...
        override fun run() {
            onRtspClientStarted(this)
            val port = if (uri.port == -1) DEFAULT_RTSP_PORT else uri.port
            val listener = createClientListener(this)
            try {
                ...
                if (currentSocket != null) {
                    ...
                    val rtspClient = RtspClient.Builder(currentSocket, uri.toString(), rtspStopped, listener)
                ...
                e.printStackTrace()
                uiHandler.post { listener.onRtspFailed(e.message) }
            } finally {
                NetUtils.closeSocket(socket)
                socket = null
            }
            onRtspClientStopped(this)
        }
    }
    ...
    private fun onRtspClientStarted(thread: RtspThread) {
...
//        uiHandler.post { statusListener?.onRtspStatusConnected() }
    ...
    private fun onRtspClientStopped(thread: RtspThread) {
